...
function guardarAlumno(event) {
  ...
  API.post(formData, "alumnos/save")
    .then((data) => {
...
function crearTabla() {
  if (objDatos.filter == "") {
    ...
    objDatos.recordsFilter = objDatos.records.filter((item) => {
      ...
      if (
        telefono.toUpperCase().search(objDatos.filter.toLocaleUpperCase()) != -1
      ) {
        return item;
      }
      if (
      ...
      if (
        genero.toUpperCase().search(objDatos.filter.toLocaleUpperCase()) != -1
      ) {
        return item;
      }
      if (
    ...
    if (index >= recordIni && index <= recordFin) {
      html += `
                    ...
                    <td>
                        ...
                        <button type="button" class="btn btn-danger btncolor" onclick="eliminarAlumno(${
                          item.id_alumno
