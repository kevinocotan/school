...
function guardarEgreso(event) {
    ...
    API.post(formData,"egresosuser/save").then(
        data=>{
...
function cargarDatos() {
    API.get("egresosuser/getAll").then(
        data=>{
...
function cargarCompraProducto() {
    API.get("compraproductosuser/getAll").then(
        data=>{
            if (data.success) {
...
function editarEgreso(id) {
    ...
    panelDatos.classList.add("d-none");
    panelForm.classList.remove("d-none");
    API.get("egresosuser/getOneEgreso?id="+id).then(
        data=>{
            if (data.success) {
...
function mostrarDatosForm(record) {
    const {id_egreso, fecha, id_compra, descripcion, monto}=record;
    ...
    Swal.fire({
        ...
        resultado=>{
            ...
            if (resultado.isConfirmed) {
                API.get("egresosuser/deleteEgreso?id="+id).then(
                    data=>{
                        if (data.success) {
                            cancelarEgreso();
                            Swal.fire({
